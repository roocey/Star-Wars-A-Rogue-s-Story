...
# Inspired by "Dwarf Fortress", "Dungeon Crawl Stone Soup", "Angband", "IVAN", "Fallout", "Rogue", "Metroid",
# "Crypt of the Necrodancer", "The Slimy Lichmummy", "The Binding of Isaac", and many others.
...
# To comply with this license, you must give prominent notice that you use the Simple DirectMedia Layer library,
# and that it is included under the terms of the LGPL license. You must provide a copy of the LGPL license.
# You must also do one of the following:
# 1. Link with the library as a shared object (e.g. SDL.dll or libSDL.so)
# 2. Provide the object or source code to your application along with any libraries and custom tools not available with
# a standard platform development kit.
# You may also simply provide a written offer, valid for three years, to provide these materials upon request to anyone
# with a legal copy of your application.
# If you include the SDL library in binary form, you should also make available the source code to the version you
# provide, including any customizations you have made.
...
TORCH_RADIUS = 8  # default 8; set to 10 if BIG BROTHER; set to 4 if CYBERDAEMON OF DARKNESS; set to 5 if BIG BROTHER
# & CYBERDAEMON OF DARKNESS
...
class Object:
    ...
    def __init__(self, x, y, char, name, color, examineMSG=None, special_flag=False, boss=False, took_turn=False,
                 effect_flag=False, NPC_flag=False, always_visible=False, blocks=False, fighter=None, PC=None, ai=None,
                 item=None, equipment=None, teleporter=None, bg_color=libtcod.black):
        self.x = x
        self.y = y
        self.char = char
        self.name = name
    ...
    def move(self, dx, dy):
        ...
        if 0 <= (self.x + dx) < MAP_WIDTH and 0 <= (self.y + dy) < MAP_HEIGHT:
            if not is_blocked(self.x + dx, self.y + dy) or (self.fighter and self.fighter.status.get('Acidic') > 0):
                if (self.ai and ('swims' in self.ai.tags and map[self.x + dx][self.y + dy].flag == 'water')) or (
                            self.ai and 'swims' not in self.ai.tags) or (not self.ai):
                    if map[self.x + dx][self.y + dy].flag != 'shop' and is_blocked(self.x + dx, self.y + dy):
                        ...
                        if self == player:
                            message('You melt through the wall.', libtcod.light_grey)
                    if map[self.x + dx][self.y + dy].flag != 'shop':
                        if self == player:
                            if map[self.x + dx][self.y + dy].flag == 'snow pile':
                                map[self.x + dx][self.y + dy] = Tile(False, False, flag=None)
                                message('The snow melts beneath you.', libtcod.light_grey)
                        self.x += dx
                        self.y += dy
                        if self.fighter:
    ...
    def draw(self):
        ...
        if libtcod.map_is_in_fov(fov_map, self.x, self.y) or (self.always_visible and map[self.x][self.y].explored) or (
                    self.fighter and self.fighter.status.get('Faerie Fire') > 0) or (
                    self.fighter and player.fighter.status.get('Telepathy') > 0):
            # set the color and then draw the character that represents this object at its position
            if libtcod.map_is_in_fov(fov_map, self.x, self.y):
                libtcod.console_put_char_ex(con, self.x, self.y, self.char, self.color, self.bg_color)
            else:
                libtcod.console_put_char_ex(con, self.x, self.y, self.char, self.color * libtcod.grey,
                                            self.bg_color * libtcod.grey)

    def clear(self):
        # erase the character that represents this object
        if libtcod.map_is_in_fov(fov_map, self.x, self.y) or (
                    self.fighter and self.fighter.status.get('Faerie Fire') > 0) or (
                    player.fighter.status.get('Telepathy') > 0):
    ...
    def move_astar(self, target):
        ...
        if (self.ai and ('swims' in self.ai.tags and map[self.x][self.y].flag == 'water')) or (
                    self.ai and 'swims' not in self.ai.tags) or (not self.ai):
...
class PC:
    def __init__(self, pp, experience, brightness, AGI, CON, DEX, FOR, STR):
    ...
    @property
    def AGI(self):
        bonus = sum(equipment.AGI for equipment in get_all_equipped(player))
        penalty = 0
        return self.base_AGI + bonus - penalty

    @property
    def CON(self):
        bonus = sum(equipment.CON for equipment in get_all_equipped(player))
        penalty = 0
        return self.base_CON + bonus - penalty

    @property
    def DEX(self):
        bonus = sum(equipment.DEX for equipment in get_all_equipped(player))
        penalty = (player.fighter.status['Stunned'])
        return self.base_DEX + bonus - penalty

    @property
    def FOR(self):
        bonus = sum(equipment.FOR for equipment in get_all_equipped(player))
        penalty = 0
        return self.base_FOR + bonus - penalty

    @property
    def STR(self):
        bonus = sum(equipment.STR for equipment in get_all_equipped(player))
        penalty = (player.fighter.status['Stunned'])
        return self.base_STR + bonus - penalty
            ...
            if thing.special_flag:
                if thing.name == 'orb':
                    ...
                    if player.fighter.hp <= 0:
                        base_message += ' You die from the fall...'
                ...
                elif thing.name == 'spaceport':
                    player.PC.interaction_checker.append('spaceport')
...
class Fighter:
    ...
    def __init__(self, hp, dmg, attack_range, dmg_clumps, last_direction=0, dmg_bonus=0, armor=0, accuracy=0,
                 describe_attack=None, death_function=None):
        self.owner = None
        self.max_mp = None
        self.base_max_hp = hp
        self.hp = hp
        ...
        self.status = {'Stuck': 0, 'Floating': 0, 'Damage Taken': 0, 'Attack Limit': 0, 'Telepathy': 0, 'Frozen': 0,
                       'Acidic': 0, 'Cursed': 0, 'Faerie Fire': 0, 'Hypothermia': 0, 'Confused': 0, 'Stunned': 0}
        self.poison_ticks = 0
    ...
    @property
    def accuracy(self):
        bonus = sum(equipment.accuracy for equipment in get_all_equipped(self.owner))
        if self.owner == player:
            bonus += player.PC.DEX
    ...
    def attack(self, target):
        ...
        if self.status.get('Frozen') == 0 and self.dmg > 0:
            ...
            clump_check = self.dmg_clumps  # number of damage dice

            dmg_check = self.dmg  # pips on damage dice

            message_color = libtcod.white

            damage = 0

            attack_chance = 60 + self.accuracy - target.fighter.armor
            ...
            if self.owner == player:
                dmg_check -= 1
                name = "You"
                if player.PC.STR >= libtcod.random_get_int(dungeon_seed, 1, 100):
                    clump_check += 1
            else:
                name = self.owner.name

            if attack_chance >= libtcod.random_get_int(dungeon_seed, 1, 100):
                if self.dmg_bonus > 0:
                ...
                while clump_check > 0:
                    ...
                    if self.owner == player:
                        ...
                        if target.fighter.invinc_ticks == 0:
                            if self.describe_attack is not None:
                                if target == player and self.owner.boss and self.owner.name != 'sand worm':
                                    if 'Duke of' in name or 'Bell Keeper' is name:
                                        message('The ' + name + ' ' + self.describe_attack + ' you' + damage_message,
                                                message_color)
                                    else:
                                        message(
                                            name.capitalize() + ' ' + self.describe_attack + ' you' + damage_message,
                                            message_color)
                                    ...
                                    message(
                                        'The ' + name + ' ' + self.describe_attack + ' the ' + target.name +
                                        damage_message,
                                        message_color)
                            else:
                ...
                if self.owner == player:
                    message('You miss.(' + str(attack_chance) + '%)', libtcod.grey)
                else:
                    message('The ' + name + ' misses.(' + str(attack_chance) + '%)',
                            libtcod.grey)
    ...
    def cast_knockback(self, knock_x, knock_y, distance):
        ...
        try:
            if knock_x > self.owner.x and knock_y == self.owner.y:
                ...
                while distance > 0:
                    self.owner.move(libtcod.random_get_int(dungeon_seed, -1, 1),
                                    libtcod.random_get_int(dungeon_seed, -1, 1))
                    distance -= 1
        except Exception:
...
class BasicMonster:
    def __init__(self, mob_flag=0, tracking=0, attack_chance=0, special_chance=0, tags=None):
        self.owner = None
        if tags is None:
            tags = []
    ...
    def move_around(self):
        ...
        try:
            while numOfMoves > 0:
                numOfMoves -= 1
                if 'shouts' not in self.tags or (
                                'shouts' in self.tags and libtcod.random_get_int(dungeon_seed, 1, 8) != 8):
                    #                    direction_multi = 1
                    #                    if libtcod.random_get_int(dungeon_seed, 1, 2) == 2:
                    ...
                    if 'flutters' in self.tags:  # mobs with the flutter flag have a 1-in-3 chance of moving around
                        # randomly after their standard move
                        if libtcod.random_get_int(dungeon_seed, 1, 3) == 3:
                            monster.move(libtcod.random_get_int(dungeon_seed, -1, 1),
                                         libtcod.random_get_int(dungeon_seed, -1, 1))
                    ...
                    if libtcod.map_is_in_fov(fov_map, monster.x, monster.y):
                        ...
                        for object in objects:
                            if object.fighter and object.effect_flag is False and object != player:
                                if object.fighter.hp > 0:
    ...
    def mob_hit(self):  # tells the mob to make an attack and triggers any "on attack" effects
        ...
        if 'nurse' not in self.tags:
            ...
            random_doctor = libtcod.random_get_int(dungeon_seed, 1,
                                                   100)
        ...
        if 'stuns' in self.tags:
            message('The ' + self.owner.name + '\'s attack stuns you.')
            player.fighter.status['Stunned'] = libtcod.random_get_int(dungeon_seed, 8, 16)
        ...
        if 'displaces' in self.tags:
            try:
                ...
                while blinks > 0:
                    blinks -= 1
                    player.move(libtcod.random_get_int(dungeon_seed, -1, 1),
                                libtcod.random_get_int(dungeon_seed, -1, 1))
            except Exception:
                logging.exception('Caught an error')
            ...
            if self.owner.x == player.x or self.owner.y == player.y:
                ...
                try:
                    while clumps > 0:
                        ...
                        for object in objects:
                            if object.fighter and object.fighter.hp > 0 and object != self.owner and object.distance_to(
                                    player) <= 2 and not (object.ai and 'breathes fire' in object.ai.tags):
                                if object == player:
                                    message('You were scorched by the ' + self.owner.name + '\'s fire for ' + str(
                                        damage_total) + '.')
                                else:
                                    message(
                                        object.name.capitalize() + ' was scorched by the ' + self.owner.name +
                                        '\'s fire for ' + str(damage_total) + '.')
    ...
    def take_turn(self):
                    ...
                    while check:
                        ...
                        if pulse != 9:
                            if map[self.owner.x + random_x][self.owner.y + random_y].flag != 'water':
                                ...
                                libtcod.map_set_properties(fov_map, self.owner.x + random_x, self.owner.y + random_y,
                                                           not map[self.owner.x + random_x][
                                                               self.owner.y + random_y].block_sight,
                                                           not map[self.owner.x + random_x][
                                                               self.owner.y + random_y].blocked)
                                check = False
                        else:
                            check = False
            except Exception:
        ...
        if 'confuses' in self.tags:
            try:
                for object in objects:
                    if object.fighter and (
                                (
                                            object.ai and 'confuses' not in object.ai.tags) or object == player) and \
                                    object.distance_to(self.owner) <= 2:
...
class Item:
    ...
    def __init__(self, use_function=None, cast_function=None, dippable=False, powerup=False, skill=False,
                 character=False, name_update=False, spell_type=None, plural_name=None, real_name=None, stacks=None,
                 charges=None, mana_cost=None, cost=None, item_type=None, wand_chance=None, stop_function=None):
        self.owner = None
        ...
        self.stacks = stacks  # allows certain types of items (e.g., scrolls & potions) to stack
        ...
        self.real_name = real_name  # used to remember the real name of an unidentified item )
        self.plural_name = plural_name  # used to give custom pluralization to stacks of items
        self.dippable = dippable  # whether or not the item can be dipped in fountains
        self.spell_type = spell_type  # the actual spell being cast by the item
    ...
    def dip(self, dipped_into=None):
        if dipped_into is not None:
            if self.dippable:
                if self.item_type == 'wand':
                    self.identify()
                    message(
                        'You dip your ' + self.owner.name + ' into the ' + dipped_into.name +
                        '. It explodes into a torrent of magical energy!',
                        libtcod.cyan)
                    wand_explosion()
                ...
                if self.item_type == 'spellbook':
                    ...
                    message(
                        'You dip your ' + self.owner.name + ' into the ' + dipped_into.name +
                        '. The magical aura surrounding the book dissipates and the pages turn blank.',
                        libtcod.cyan)
                    inventory.remove(self.owner)
                    create_item('blank spellbook', player.x, player.y, 'start')
                    render_all()
                if self.item_type == 'scroll':
                    self.identify()
                    message(
                        'You dip your ' + self.owner.name + ' into the ' + dipped_into.name +
                        '. The text fades away and all that remains is a blank scroll.',
                        libtcod.cyan)
                    if self.stacks > 1:
                        self.stacks -= 1
                    else:
                        inventory.remove(self.owner)
                    create_item('blank scroll', player.x, player.y, 'start')
                    render_all()
    ...
    def magic(self):
        ...
        if self.spell_type == 'striking':  # attacks a distant for damage equal to the player's current combat step
            if monster is not None:
                ...
                damage_value = libtcod.random_get_int(dungeon_seed, 1,
                                                      ((player.fighter.dmg_clumps * player.fighter.dmg) - 1) * 2)

                monster.fighter.take_damage(damage_value)

                message(base_message + str(damage_value) + '.', libtcod.white)

        elif self.spell_type == 'pacification':  # makes all enemies unable to track the the player for 15-30 turns
            #  and increases their chance to do something special (usually: "do nothing")
            for object in objects:
                if object.fighter and object.fighter.hp > 0 and object.ai is not None:
            ...
            if player.fighter.last_direction == 'up':
                ...
                base_message += ' The bolt strikes the ground beneath you. A pit appears!'
                ...
                for object in objects:
                    if object.ai and object.distance_to(monster) <= 2:
                        if 'cancelled' in object.ai.tags:
                            if object.fighter:
                                object.fighter.take_damage(
                                    object.fighter.hp * 6)  # kill enemies that have the 'cancelled' tag
                        del object.ai.tags[:]
    ...
    def use(self):
        if self.use_function is None and self.mana_cost is None:
            message('The ' + self.owner.name + ' cannot be used.')
        else:
            if (player.PC.pp >= self.mana_cost) or (self.mana_cost is None):
                ...
                if self.item_type != 'spellbook' and self.item_type != 'wand' and self.item_type != 'misc':
                    if self.stacks is None and self.charges is None:
                        inventory.remove(self.owner)
                    elif self.stacks > 1:
                        self.stacks -= 1
                    elif self.stacks == 1:
            ...
            elif self.mana_cost is not None and self.mana_cost > player.PC.pp:
                message('You don\'t have enough magical power to use the ' + self.owner.name + '.', libtcod.white)


def wand_explosion():
    for object in objects:
        if libtcod.map_is_in_fov(fov_map, object.x,
                                 object.y) and object.fighter and object.fighter.hp > 0 and object.effect_flag is False:
...
class Equipment:
    ...
    def __init__(self, slot, dmg=0, dmg_clumps=0, enchant_level=0, max_hp=0, max_mp=0, max_pp=0, cost=0, attack_range=0,
                 armor=0, brightness=0, accuracy=0, AGI=0, CON=0, DEX=0, FOR=0, STR=0, use_function=None):
        self.owner = None
        self.dmg = dmg
...
def create_h_tunnel(x1, x2, y, flavor=None):
    ...
    for x in range(min(x1, x2), max(x1, x2) + 1):
        if 0 <= x - 1 < MAP_WIDTH:
            if flavor is None:
                map[x][y] = Tile(False, False)
            elif flavor == 'grass' and libtcod.random_get_int(dungeon_seed, 1, 2) != 1:
                map[x][y] = Tile(False, True, flag='grass')
...
def make_map(flavor=None):
    global map, objects, stairs, BOSS_COUNT, dungeon_level, MAP_HEIGHT, MAP_WIDTH, dungeon_seed, wallColor, \
        wallColors, mobs_in_sight
    objects = [player]
    mobs_in_sight = []
    BOSS_COUNT = 0
                        ...
                        if (dungeon_level%2 == 0):
                            for y in range(new_y - 1, new_y + 1):
                                for x in range(new_x - 1, new_x + 1):
                                    map[x][y] = Tile(False, False, flag=None)
                            create_special('spaceport', new_x, new_y)
                    if num_rooms == 2:
...
def place_objects(room, no_monsters, flag=None):
    ...
    monster_chances = {}
    monster_chances['probe droid'] = from_dungeon_level([[10000, 1]])
    monster_chances['tauntaun'] = from_dungeon_level([[10000, 1]])
    monster_chances['wampa'] = from_dungeon_level([[5000, 1]])
...
def render_bar(x, y, total_width):
    ...
    libtcod.console_print_ex(panel, x+1, y-1, libtcod.BKGND_NONE, libtcod.CENTER, player.PC.active_planet)

    libtcod.console_set_default_foreground(panel, libtcod.white)
    libtcod.console_print_ex(panel, x, y+1, libtcod.BKGND_NONE, libtcod.CENTER, 'HP')
    ...
    libtcod.console_print_ex(panel, x + 3, y+1, libtcod.BKGND_NONE, libtcod.CENTER, str(player.fighter.hp))

    libtcod.console_set_default_foreground(panel, libtcod.white)
    libtcod.console_print_ex(panel, x, y + 2, libtcod.BKGND_NONE, libtcod.CENTER, 'FP')
    libtcod.console_set_default_foreground(panel, libtcod.green)
    libtcod.console_print_ex(panel, x + 3, y + 2, libtcod.BKGND_NONE, libtcod.CENTER, str(player.PC.pp))

    libtcod.console_set_default_foreground(panel, libtcod.white)
    libtcod.console_print_ex(panel, x, y + 3, libtcod.BKGND_NONE, libtcod.CENTER, 'XL')
    libtcod.console_print_ex(panel, x + 3, y + 3, libtcod.BKGND_NONE, libtcod.CENTER, str(player.PC.experience))

    libtcod.console_print_ex(panel, x, y + 5, libtcod.BKGND_NONE, libtcod.CENTER, 'AC')
    libtcod.console_print_ex(panel, x + 3, y + 5, libtcod.BKGND_NONE, libtcod.CENTER, str(player.fighter.armor))

    libtcod.console_print_ex(panel, x, y + 6, libtcod.BKGND_NONE, libtcod.CENTER, 'TH')
    libtcod.console_print_ex(panel, x + 3, y + 6, libtcod.BKGND_NONE, libtcod.CENTER, str(player.fighter.accuracy))

    libtcod.console_print_ex(panel, x, y + 8, libtcod.BKGND_NONE, libtcod.CENTER, 'CON')
    extend = 0
    if len(str(player.PC.CON)) >= 3:
        extend += 1
    libtcod.console_print_ex(panel, x + 4 + extend, y + 8, libtcod.BKGND_NONE, libtcod.CENTER, str(player.PC.CON) + '%')

    libtcod.console_print_ex(panel, x, y + 9, libtcod.BKGND_NONE, libtcod.CENTER, 'DEX')
    extend = 0
    if len(str(player.PC.DEX)) >= 3:
        extend += 1
    libtcod.console_print_ex(panel, x + 4 + extend, y + 9, libtcod.BKGND_NONE, libtcod.CENTER, str(player.PC.DEX) + '%')

    libtcod.console_print_ex(panel, x, y + 10, libtcod.BKGND_NONE, libtcod.CENTER, 'FOR')
    extend = 0
    if len(str(player.PC.FOR)) >= 3:
        extend += 1
    libtcod.console_print_ex(panel, x + 4 + extend, y + 10, libtcod.BKGND_NONE, libtcod.CENTER, str(player.PC.FOR) + '%')

    libtcod.console_print_ex(panel, x, y + 11, libtcod.BKGND_NONE, libtcod.CENTER, 'STR')
    extend = 0
    if len(str(player.PC.STR)) >= 3:
        extend += 1
    libtcod.console_print_ex(panel, x + 4 + extend, y + 11, libtcod.BKGND_NONE, libtcod.CENTER, str(player.PC.STR) + '%')

    if player.fighter.status.get('Acidic') > 0:
        libtcod.console_set_default_foreground(panel, libtcod.chartreuse)
        libtcod.console_print_ex(panel, x + 4, y + 14 + len(mobs_in_sight), libtcod.BKGND_NONE, libtcod.CENTER,
                                 'Acidic')

    if player.fighter.status.get('Confused') > 0:
        libtcod.console_set_default_foreground(panel, libtcod.yellow)
        libtcod.console_print_ex(panel, x + 4, y + 15 + len(mobs_in_sight), libtcod.BKGND_NONE, libtcod.CENTER,
                                 'Confused')

    if player.fighter.status.get('Stuck') > 0:
        libtcod.console_set_default_foreground(panel, libtcod.cyan)
        libtcod.console_print_ex(panel, x + 4, y + 16 + len(mobs_in_sight), libtcod.BKGND_NONE, libtcod.CENTER, 'Stuck')
    ...
    for monster in mobs_in_sight:
        ...
        libtcod.console_print_ex(panel, x - 1, y + 13 + monster_num, libtcod.BKGND_SET, libtcod.CENTER,
                                 str(monster.char))

        health_display = health_pips_counter(monster)

        libtcod.console_set_default_foreground(panel, libtcod.white)
        libtcod.console_print_ex(panel, x + 4, y + 13 + monster_num, libtcod.BKGND_NONE, libtcod.CENTER,
                                 str(health_display))
...
def render_all():
    ...
    global color_dark_ground, color_light_ground, dungeon_level, fov_recompute
    if fov_recompute:
        ...
        for y in range(MAP_HEIGHT):
            for x in range(MAP_WIDTH):
                ...
                if (player.PC.active_planet == 'Hoth'):
                    wallColor = libtcod.cyan
                    if map[x][y].flag == 'grass':
                        map[x][y].flag = 'snow pile'

                visible = libtcod.map_is_in_fov(fov_map, x, y)
                wall = map[x][y].block_sight and map[x][y].flag is None and map[x][y].blocked
                grass = map[x][y].flag == 'grass'
                snow_pile = map[x][y].flag == 'snow pile'
                ...
                if not visible:
                    ...
                    if map[x][y].explored:
                        if shop:
                            libtcod.console_put_char_ex(con, x, y, '=', libtcod.Color(255, 215, 0) * libtcod.grey,
                                                        libtcod.black)
                        elif wall:
                            if player.fighter.status.get('Acidic') > 0:
                            ...
                            libtcod.console_put_char_ex(con, x, y, '"', wallColor * libtcod.lightest_grey,
                                                        libtcod.black)
                        elif snow_pile:
                            libtcod.console_put_char_ex(con, x, y, '*', wallColor * libtcod.lightest_grey,
                                                        libtcod.black)
                            ...
                            libtcod.console_put_char_ex(con, x, y, '~',
                                                        libtcod.Color(libtcod.random_get_int(0, 120, 180),
                                                                      libtcod.random_get_int(0, 120, 180),
                                                                      libtcod.random_get_int(0, 120,
                                                                                             180)) * libtcod.grey,
                                                        libtcod.blue * libtcod.random_get_float(0, 0.33, 0.67))
                        else:
                            libtcod.console_put_char_ex(con, x, y, '.', libtcod.Color(238, 238, 238) * libtcod.grey,
                                                        libtcod.black)

                else:
                    # it's visible
                    if shop:
                        libtcod.console_put_char_ex(con, x, y, '=', libtcod.Color(libtcod.random_get_int(0, 230, 255),
                                                                                  libtcod.random_get_int(0, 200, 215),
                                                                                  0), libtcod.black)
                    elif wall:
                        if player.fighter.status.get('Acidic') > 0:
                            libtcod.console_put_char_ex(con, x, y, '=', libtcod.black,
                                                        libtcod.light_chartreuse * libtcod.random_get_float(0, 0.67,
                                                                                                            1.33))
                        else:
                            libtcod.console_put_char_ex(con, x, y, '=', wallColor *
                                                        libtcod.random_get_float(0, 1.0, 1.2),
                                                        libtcod.black)
                    elif corpse_red:
                        ...
                        libtcod.console_put_char_ex(con, x, y, '"', wallColor*libtcod.random_get_float(0, 0.9, 1.5),
                                                    libtcod.black)
                    elif snow_pile:
                        libtcod.console_put_char_ex(con, x, y, '*', wallColor*libtcod.random_get_float(0, 0.9, 1.5),
                                                    libtcod.black)
                    elif crystal:
                        libtcod.console_put_char_ex(con, x, y, 30, wallColor*libtcod.random_get_float(0, 1.0, 1.5) *
                                                    libtcod.light_cyan, libtcod.black)
                    elif water:
                        libtcod.console_put_char_ex(con, x, y, '~', libtcod.Color(libtcod.random_get_int(0, 120, 180),
                                                                                  libtcod.random_get_int(0, 120, 180),
                                                                                  libtcod.random_get_int(0, 120, 180)),
                                                    libtcod.blue * libtcod.random_get_float(0, 0.67, 1.33))
                    else:
...
def message(new_msg, color=libtcod.white):
    ...
    for line in new_msg_lines:
        ...
        if data.suppress_msg_spam and color == libtcod.light_grey:
            try:
                if game_msgs[-1] == game_msgs[-2]:
                    del game_msgs[-1]
            except IndexError:
                return
...
def player_move_or_attack(dx, dy):
    ...
    target = None
    for object in objects:
        if (object.fighter or object.special_flag) and (
                    (object.x == x_attack and object.y == y_attack) or (object.x == x and object.y == y)):
            target = object
            break
...
def inventory_menu(header, dip=False):
    ...
    if len(inv_merged) == 0:
        ...
        if dip:
            for item in inv_merged:
                if item.item.dippable:
                    text = item.name
                    if item.item.stacks > 1:
                        if item.item.plural_name is not None:
                            if item.item.mana_cost is None:
                                text = str(item.item.stacks) + ' ' + item.item.plural_name
                            else:
                                if item.name != item.item.real_name:
                                    text = 'x' + str(item.item.stacks) + ' ' + item.name
                                else:
                                    text = str(item.item.stacks) + ' ' + item.item.plural_name + ' {' + str(
                                        item.item.mana_cost) + '}'
                        else:
                            text = 'x' + str(item.item.stacks) + ' ' + item.name
                    elif item.item.mana_cost is not None and item.item.wand_chance is None:
                        if item.name != item.item.real_name:  # check if identified
                            text = item.name
                        else:
                            text = item.name + ' {' + str(item.item.mana_cost) + '}'
                    elif item.item.mana_cost is not None and item.item.wand_chance is not None:
                        if item.name != item.item.real_name:  # check if identified
                            text = item.name
                        else:
                            text = item.name + ' {' + str(item.item.mana_cost) + ', ' + str(
                                item.item.wand_chance) + '%' + '>}'
            ...
            for item in inv_merged:
                ...
                if item.item.stacks > 1:
                    if item.item.plural_name is not None:
                        ...
                        text = 'x' + str(item.item.stacks) + ' ' + item.name
                elif item.item.mana_cost is not None and item.item.wand_chance is None:  # spellbooks
                    if item.name != item.item.real_name:  # check if identified
                        text = item.name
                    else:
                        text = item.name + ' {' + str(item.item.mana_cost) + '}'
                elif item.item.mana_cost is not None and item.item.wand_chance is not None:  # wands
                    if item.name != item.item.real_name:  # check if identified
                        text = item.name
                    else:
                        text = item.name + ' {' + str(item.item.mana_cost) + ', ' + str(
                ...
                if item.item.stacks == 0 or item.item.stacks is None:
                    if item.equipment:
    ...
    if index is None or len(inv_merged) == 0:
        return None
...
def handle_keys():
    ...
    try:
        ...
        if key.vk == libtcod.KEY_F1:
        ...
        elif key.vk == libtcod.KEY_F1 and WIZ_MODE is True:
            message('Wiz mode: moving to level one.', libtcod.gold)
            player.PC.old_dungeon_level = dungeon_level
            dungeon_level = 0
            next_level()

        elif key.vk == libtcod.KEY_F2 and WIZ_MODE is True:
            message('Wiz mode: moving to level two.', libtcod.gold)
            player.PC.old_dungeon_level = dungeon_level
            dungeon_level = 1
            next_level()

        elif key.vk == libtcod.KEY_F3 and WIZ_MODE is True:
            message('Wiz mode: moving to level two.', libtcod.gold)
            player.PC.old_dungeon_level = dungeon_level
            dungeon_level = 1
            next_level()

        elif key.vk == libtcod.KEY_F4 and WIZ_MODE is True:
            message('Wiz mode: moving to level four.', libtcod.gold)
            dungeon_level = 3
            next_level()

        elif key.vk == libtcod.KEY_F5 and WIZ_MODE is True:
            message('Wiz mode: moving to level six.', libtcod.gold)
            dungeon_level = 5
            next_level()

        elif key.vk == libtcod.KEY_F6 and WIZ_MODE is True:
            message('Wiz mode: moving to level ten.', libtcod.gold)
            dungeon_level = 9
            next_level()

        elif key.vk == libtcod.KEY_F7 and WIZ_MODE is True:
            message('Wiz mode: moving to level seven.', libtcod.gold)
            dungeon_level = 22
            next_level()

        elif key.vk == libtcod.KEY_F11 and WIZ_MODE is True:
            player.fighter.hp = player.fighter.max_hp
            message('Wiz mode: hp set to full.', libtcod.gold)

        elif key.vk == libtcod.KEY_F12 and WIZ_MODE is True:
            ...
            message('Created by Andrew Wright (@roocey) for 7DRL 2016. Developed from 3/5/16 to 3/12/16.',
                    libtcod.orange)
                ...
                if key_char == '?' or key_char == '/':
                    ...
                    while help_choice is None:
                        ...
                        if help_choice == 0:
                            ...
                            message(
                                'You can move by pressing <wasd>. You can wait a turn by pressing <SPACE>. '
                                'Alternatively, you may use the arrow keys or the numpad.',
                                libtcod.light_green)
                        elif help_choice == 1:
                            message('----', libtcod.cyan)
                            ...
                            message('----', libtcod.cyan)
                            message('You will automatically win when you collect your 27th orb of experience.',
                                    libtcod.light_green)
                        elif help_choice == 3:
                            message('----', libtcod.cyan)
                            message(
                                'The game automatically saves periodiocally. '
                                'The game will attempt to load a savefile on launch.',
                                libtcod.light_green)
                        elif help_choice == 4:
                            message('----', libtcod.cyan)
                            message(
                                'For regular updates and information visit the developer\'s '
                                'website here: caffeineoverdose.me',
                                libtcod.light_green)
...
def closest_monster(max_range, direction=None):
    ...
    if direction is None:
        ...
        for object in objects:
            if object.fighter and not object == player and object.effect_flag is False and libtcod.map_is_in_fov(
                    fov_map, object.x, object.y) and object.fighter.hp > 0:
            ...
            if object.fighter and not object == player and libtcod.map_is_in_fov(fov_map, object.x, object.y):
                if player.x == object.x:
                    if player.y > object.y:
                        if object.effect_flag is False:
                            if object.fighter.hp > 0:
                                dist = player.distance_to(object)
...
def closest_object(max_range, direction=None):
    ...
    if direction is None:
        ...
        for object in objects:
            if not object == player and object.effect_flag is False and libtcod.map_is_in_fov(fov_map, object.x,
                                                                                              object.y):
                # calculate distance between this object and the player
                dist = player.distance_to(object)
                if dist < closest_dist:  # it's closer, so remember it
        ...
        for object in objects:
            if not object == player and libtcod.map_is_in_fov(fov_map, object.x, object.y):
                if player.x == object.x:
                    if player.y > object.y:
                        if object.effect_flag == False:
                            dist = player.distance_to(object)
                            if dist < closest_dist:
                                closest_enemy = object
                                closest_dist = dist
...
def cast_examine():
    ...
    if thing is not None:
        if thing.examineMSG is not None:
            if thing.item:
                if thing.item.item_type == 'wand' or thing.item.item_type == 'spellbook' or \
                                thing.item.item_type == 'scroll':
...
def create_monster(name, x, y):
    ...
    if 0 <= x < MAP_WIDTH and 1 <= y < MAP_HEIGHT:
        ...
        if name == 'probe droid':
            fighter_component = Fighter(hp=8, dmg_clumps=1, dmg=6, attack_range=2, describe_attack='blasts')
            ai_component = BasicMonster(tags=['wanders', 'flutters'], special_chance=25)

            monster = Object(x, y, 'p', name, libtcod.lighter_grey,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='A probe droid: an imperial scouting droid.')

        if name == 'tauntaun':
            fighter_component = Fighter(hp=10, dmg_clumps=1, dmg=6, attack_range=1, describe_attack='claws')
            ai_component = BasicMonster(tags=['wanders'], special_chance=10)

            monster = Object(x, y, 't', name, libtcod.white,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='A tauntaun: a non-sentient lizard that has adapted to the cold.')

        if name == 'wampa':
            fighter_component = Fighter(hp=16, dmg_clumps=2, dmg=6, attack_range=1, describe_attack='bites')
            ai_component = BasicMonster(tags=['wanders', 'stuns'], special_chance=33)

            monster = Object(x, y, 'W', name, libtcod.white,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='A wampa: a tall, white carnivore that would happily devour you.')
        ...
        if name == 'alchemical monstrosity':
            ...
            monster = Object(x, y, 'a', name, libtcod.fuchsia,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='An alchemical monstrosity: an alchemist that decided to experiment on itself.')
            ...
            monster = Object(x, y, 'b', name, libtcod.lightest_blue,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='A big bat: an unusually large bat.')
            ...
            monster = Object(x, y, 'c', name, libtcod.orange,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='A centaur: a half-man, half-horse armed with a bow.')
            ...
            monster = Object(x, y, 'D', name, libtcod.red,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='A dragon: a fearsome beast of legend that breathes fire and hoards treasure.')
            ...
            monster = Object(x, y, 'e', name, libtcod.sea,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='An eldritch thing: a faceless humanoid that seems to '
            ...
            monster = Object(x, y, 'f', name, libtcod.white,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='A fiend: a cowardly demon with razor sharp claws.')
            ...
            monster = Object(x, y, 'g', name, libtcod.darker_grey,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='A goblin: a little green humanoid armed with a variety of digging tools.')
            ...
            monster = Object(x, y, 'h', name, libtcod.fuchsia,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='A hobgoblin: a thick, brutish creature that packs a mean punch.')
            ...
            monster = Object(x, y, 'i', name, libtcod.cyan,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='An ice beast: a slow-moving monster that annihilates its foes.')
            ...
            monster = Object(x, y, 'j', name, libtcod.pink,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='A jelly: a curious gelatinous critter that likes to eat.')
            ...
            monster = Object(x, y, 'k', name, libtcod.white,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='A kiwi: a flightless bird from a faraway land.')
            ...
            monster = Object(x, y, 'L', name, libtcod.light_cyan,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='A lich: an undead mage that will zap you from afar.')
            ...
            monster = Object(x, y, 'm', name, libtcod.sea,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='A maiden of the lake: a waterbound humanoid with sharp claws and '
                                        'the ability to zap from afar.')
            ...
            monster = Object(x, y, 'n', name, libtcod.white,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='A nurse: an unexpected friend.')
            ...
            monster = Object(x, y, 'O', name, libtcod.yellow,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='An ogre: a simple brute with great strength.')
            ...
            monster = Object(x, y, 'p', name, libtcod.purple,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='A purple toad: a giant amphibian that confuses anything that gets too close.')
            ...
            monster = Object(x, y, 'Q', name, libtcod.azure,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='A quantum mechanic: a strange creature from another dimension.')
            ...
            monster = Object(x, y, 'r', name, libtcod.crimson,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='A redcap: an impossibly quick humanoid armed with a sharp pike.')
            ...
            monster = Object(x, y, 's', name, libtcod.darker_grey,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='A shadow: a fleeting spirit that only wants to consume so it can '
                                        'return to life.')
            ...
            monster = Object(x, y, 'T', name, libtcod.lime,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='A troll: a big brute with long claws and the ability to rapidly regenerate.')
            ...
            monster = Object(x, y, 'U', name, libtcod.white,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='An unicorn: a fast, evasive horse that can regenerate rapidly '
                                        'thanks to its legendary horn.')

        elif name == 'vortex':
            fighter_component = Fighter(hp=6, dmg_clumps=2, dmg=4, attack_range=1, describe_attack='shocks')
            ai_component = BasicMonster(tags=['wanders', 'mana burns', 'crawls', 'flutters', 'cancelled'],
                                        special_chance=20)

            monster = Object(x, y, 'v', name, libtcod.light_violet,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='A vortex: a whirling mass of energy that destroys magical power.')
            ...
            monster = Object(x, y, 'w', name, libtcod.light_blue,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='A will-o\'-the-wisp: a curious dancing flame rumored to breathe '
                                        'fire when in danger.')
            ...
            monster = Object(x, y, 'X', name, libtcod.copper,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='A xavite: an elemental being composed of dirt and stone that '
                                        'will pin its prey in place.')
            ...
            monster = Object(x, y, 'y', name, libtcod.amber,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='A yacuruna: a devious water spirit armed with a spear and a bad attitude.')
            ...
            monster = Object(x, y, 'z', name, libtcod.green,
                             blocks=True, fighter=fighter_component, ai=ai_component,
                             examineMSG='A zombie: a shambling corpse that refuses to accept death.')

    if monster is not None:
        objects.append(monster)
        monster.fighter.death_function = monster_death

        monster.fighter.max_hp = libtcod.random_get_int(dungeon_seed, monster.fighter.max_hp,
                                                        int(monster.fighter.max_hp * 1.5))
...
def create_special(name, x, y):  # creates special dungeon objects that are not items or fighters (e.g., fountains)
    ...
    if name == 'orb':
        ...
        special = Object(x, y, 127, name, libtcod.light_green, blocks=True)
...
def create_item(name, x, y, flavor=None):
    ...
    random_spellbooks = ['spellbook of striking', 'spellbook of cancellation', 'spellbook of digging',
                         'spellbook of healing']

    random_rings = ['ring of telepathy', 'ring of vampirism', 'ring of purity', 'ring of the mageblade',
    ...
    if name == 'dagger':
        ...
        equipment = Object(x, y, '/', name, libtcod.lighter_cyan,
                           examineMSG='A dagger: an elegant weapon from a simpler time.', equipment=equipment_component)

    if name == 'sword':
        equipment_component = Equipment('weapon', dmg_clumps=2, dmg=6, enchant_level=enchant_random, attack_range=1,
                                        cost=8)
        equipment = Object(x, y, '/', name, libtcod.cyan, examineMSG='A sword: similar to two daggers stuck together.',
                           equipment=equipment_component)

    if name == 'greatsword':
        equipment_component = Equipment('weapon', dmg_clumps=3, dmg=6, enchant_level=enchant_random, attack_range=1,
                                        cost=8)
        equipment = Object(x, y, '/', name, libtcod.darker_cyan,
                           examineMSG='A greatsword: similar to four daggers stuck together. Or two swords.',
                           equipment=equipment_component)

    if name == 'spear':
        equipment_component = Equipment('weapon', dmg_clumps=1, dmg=8, enchant_level=enchant_random, attack_range=2,
                                        cost=8)
        equipment = Object(x, y, '/', name, libtcod.lighter_orange,
                           examineMSG='A spear: put simply, a pointy stick. Can attack from up to two tiles away.',
                           equipment=equipment_component)

    if name == 'pike':
        equipment_component = Equipment('weapon', dmg_clumps=1, dmg=12, enchant_level=enchant_random, attack_range=2,
                                        cost=8)
        equipment = Object(x, y, '/', name, libtcod.orange,
                           examineMSG='A pike: the weapon of choice for the overly cautious fighter. '
                                      'Can attack from up to two tiles away.',
                           equipment=equipment_component)

    if name == 'hammer':
        equipment_component = Equipment('weapon', dmg_clumps=4, dmg=3, enchant_level=enchant_random, attack_range=1,
                                        cost=8)
        equipment = Object(x, y, '/', name, libtcod.flame,
                           examineMSG='A hammer: a mighty weapon if you want to bash some skulls in.',
                           equipment=equipment_component)

    if name == 'war hammer':
        equipment_component = Equipment('weapon', dmg_clumps=4, dmg=4, enchant_level=enchant_random, attack_range=1,
                                        cost=8)
        equipment = Object(x, y, '/', name, libtcod.darker_flame,
                           examineMSG='A war hammer: an even mightier weapon if you want to bash even more skulls in.',
                           equipment=equipment_component)

    if name == 'ring of telepathy':
        equipment_component = Equipment('ring', cost=8)
        equipment = Object(x, y, '=', name, libtcod.cyan,
                           examineMSG='A ring of telepathy: a glimpse of things to come.',
                           equipment=equipment_component)

    if name == 'ring of vampirism':
        equipment_component = Equipment('ring', cost=8)
        equipment = Object(x, y, '=', name, libtcod.crimson, examineMSG='A ring of vampirism: a taste for blood.',
                           equipment=equipment_component)

    if name == 'ring of purity':
        equipment_component = Equipment('ring', cost=8)
        equipment = Object(x, y, '=', name, libtcod.white, examineMSG='A ring of purity: a commitment to virtue.',
                           equipment=equipment_component)

    if name == 'ring of the mageblade':
        equipment_component = Equipment('ring', cost=8)
        equipment = Object(x, y, '=', name, libtcod.han, examineMSG='A ring of the mageblade: a taste for mana.',
                           equipment=equipment_component)

    if name == 'ring of the assassin':
        equipment_component = Equipment('ring', cost=8)
        equipment = Object(x, y, '=', name, libtcod.grey, examineMSG='A ring of the assassin: a commitment to shadow.',
                           equipment=equipment_component)
    ...
    if 'healing' in name:
        item_component = Item(item_type=type_check, mana_cost=5 * mp_multi, real_name=name, spell_type='healing',
                              wand_chance=wand_check, plural_name=plural_check, stacks=stack_check)
        item = Object(x, y, symbol, name, unID_colors[0], examineMSG=examine_start + 'mend old wounds.',
                      item=item_component)
        ...
        item_component = Item(item_type=type_check, mana_cost=6 * mp_multi, real_name=name, spell_type='striking',
                              wand_chance=wand_check, plural_name=plural_check, stacks=stack_check)
        item = Object(x, y, symbol, name, unID_colors[1], examineMSG=examine_start + 'lash out at a distant foe.',
                      item=item_component)
        ...
        item_component = Item(item_type=type_check, mana_cost=4 * mp_multi, real_name=name, spell_type='cancellation',
                              wand_chance=wand_check, plural_name=plural_check, stacks=stack_check)
        item = Object(x, y, symbol, name, unID_colors[2], examineMSG=examine_start + 'make your foes uninteresting.',
                      item=item_component)
        ...
        item_component = Item(item_type=type_check, mana_cost=3 * mp_multi, real_name=name, spell_type='digging',
                              wand_chance=wand_check, plural_name=plural_check, stacks=stack_check)
        item = Object(x, y, symbol, name, unID_colors[3], examineMSG=examine_start + 'find safe passage.',
                      item=item_component)
    ...
    if item is not None:
        ...
        if item.item.plural_name is not None and item.item.spell_type is not None:
            item.item.plural_name = item.item.plural_name + item.item.spell_type

        if item.item.item_type == 'wand' or item.item.item_type == 'spellbook' or item.item.item_type == 'potion' or \
                        item.item.item_type == 'scroll':
...
def new_game(daily=False):
    global player, dungeon_level, merged_items, unID_colors, game_msgs, game_state, turncount, playcount, \
        dungeon_seed, dukes, inventory, wallColors, unID_wands, identified_objects, unID_books, unID_scrolls, \
        mobs_in_sight

    fighter_component = Fighter(hp=30, dmg=1, dmg_clumps=0, armor=0, attack_range=1, death_function=player_death)
    PC_component = PC(pp=30, experience=0, AGI=0, CON=0, DEX=0, FOR=0, STR=0, brightness=16)
    player = Object(0, 0, data.player_tile, 'Han',
                    libtcod.Color(data.player_color_r, data.player_color_g, data.player_color_b), blocks=True,
                    PC=PC_component, fighter=fighter_component)
    ...
    inventory = []
    mobs_in_sight = []
    unID_wands = ['oak wand', 'gold wand', 'silver wand', 'yew wand', 'sandalwood wand', 'ash wand', 'obsidian wand',
                  'pounamu wand']
    unID_books = ['odd spellbook', 'soft spellbook', 'firm spellbook', 'arcane spellbook', 'mysterious spellbook',
                  'ancient spellbook']
    unID_scrolls = ['dried gum scroll', 'papyrus scroll', 'vellum scroll', 'sandpaper scroll', 'leather scroll']
    unID_colors = [libtcod.yellow, libtcod.red, libtcod.pink, libtcod.blue, libtcod.copper, libtcod.orange,
                   libtcod.cyan, libtcod.light_violet]
    identified_objects = []
    random.shuffle(unID_wands)
    random.shuffle(unID_books)
    random.shuffle(unID_scrolls)
    random.shuffle(unID_colors)
    wallColors = [libtcod.Color(136, 204, 238), libtcod.Color(221, 204, 119), libtcod.Color(204, 102, 119),
                  libtcod.Color(170, 68, 153), libtcod.Color(221, 119, 204), libtcod.Color(68, 170, 153)]
    ...
    message(
        'You\'ve woken up on the icy planet Hoth. You must find the abandoned rebel base and escape via the spaceport.',
        libtcod.cyan)
    ...
    try:
        while not libtcod.console_is_window_closed():
            ...
            while 'temple' in player.PC.interaction_checker:
                ...
                if level_choice == 0:
                    ...
                    if confirm_choice:
                        ...
                        render_all()
                        message('You spend some time at the temple. You gained ' + str(value) + ' HP and ' + str(
                            value2) + ' MP.', libtcod.cyan)
                        player.PC.experience -= 1
                    else:
                        player.PC.interaction_checker.remove('temple')
                        render_all()
            ...
            while 'blacksmith' in player.PC.interaction_checker:
                ...
                if level_choice == 0:
                    ...
                    if weapon is not None:
                        message('You\'ve asked the smith to upgrade your weapon. Your +' + str(
                            weapon.enchant_level) + ' ' + weapon.owner.name + ' will gain 1 to 3 levels.')
                        render_all()
                        confirm_choice = confirmation_box()
                        if confirm_choice:
                            if libtcod.random_get_int(dungeon_seed, 1, 3) != 3:
                            ...
                            message('You spend some time at the blacksmith. Your +' + str(
                                weapon.enchant_level - value) + ' ' + weapon.owner.name + ' is now +' + str(
                                weapon.enchant_level) + '.')
                        else:
                            player.PC.interaction_checker.remove('blacksmith')
                            render_all()
                    else:
                    ...
                    message(
                        'You\'ve asked the smith to forge a ring. The ring she will craft is entirely random and '
                    ...
                    if confirm_choice:
                        ...
                        player.PC.experience -= 1
                        message(
                            'You spend some time at the blacksmith. She crafts a ring and sets it down next to you.')
                    else:
                        player.PC.interaction_checker.remove('blacksmith')
                        render_all()
                elif level_choice == 2:
            ...
            while 'training grounds' in player.PC.interaction_checker:
                ...
                if level_choice == 0:
                    message(
                        'You\'ve asked your fitness coach to practice agility training with you. '
                        'This exercise will raise your agility from ' +
                        str(player.PC.agility) + '% to ' + str(player.PC.agility + 10 - (player.PC.agility // 9)) +
                        '%.')
                    render_all()
                    confirm_choice = confirmation_box()
                    if confirm_choice:
                        ...
                        message('You spend some time with your fitness coach. Your agility is now ' + str(
                            player.PC.agility) + '%.')
                    else:
                        player.PC.interaction_checker.remove('training grounds')
                        render_all()
                    ...
                    message(
                        'You\'ve asked your fitness coach to practice dexterity training with you. This exercise will '
                        'raise your dexterity from ' +
                        str(player.PC.dexterity) + '% to ' + str( player.PC.dexterity + 10 -
                                                                  (player.PC.dexterity // 9)) + '%.')
                    render_all()
                    confirm_choice = confirmation_box()
                    if confirm_choice:
                        ...
                        message('You spend some time with your fitness coach. Your dexterity is now ' + str(
                            player.PC.dexterity) + '%.')
                    else:
                        player.PC.interaction_checker.remove('training grounds')
                        render_all()
                    ...
                    message(
                        'You\'ve asked your fitness coach to practice strength training with you. This exercise will '
                        'raise your strength from ' + str(player.PC.strength) + '% to ' + str(player.PC.strength + 10 -
                                                                                              (player.PC.strength // 9))
                        + '%.')
                    render_all()
                    confirm_choice = confirmation_box()
                    if confirm_choice:
                        ...
                        message('You spend some time with your fitness coach. Your strength is now ' + str(
                            player.PC.strength) + '%.')
                    else:
                        player.PC.interaction_checker.remove('training grounds')
                        render_all()
            ...
            if game_state == 'playing' and player_action != 'didnt-take-turn':
                ...
                for object in objects:
                    if object.ai and object.took_turn is False:
...
def main_menu():
    ...
    while not libtcod.console_is_window_closed():
        ...
        libtcod.console_print_ex(0, SCREEN_WIDTH / 2, SCREEN_HEIGHT - 37, libtcod.BKGND_NONE, libtcod.CENTER,
                                 'Crown of Rogues')
        ...
        libtcod.console_print_ex(0, SCREEN_WIDTH / 2, SCREEN_HEIGHT - 1, libtcod.BKGND_NONE, libtcod.CENTER,
                                 'By Andrew Wright')
...
def load_game():
    ...
    global map, objects, player, merged_items, game_msgs, game_state, dungeon_level, dungeon_seed, turncount, \
        inventory, MAP_HEIGHT, MAP_WIDTH, mobs_in_sight, wallColors, unID_wands, unID_scrolls, unID_books, \
        unID_colors, identified_objects, wallColor
...
libtcod.console_set_custom_font('dat/fonts/' + data.font, libtcod.FONT_TYPE_GREYSCALE | libtcod.FONT_LAYOUT_ASCII_INROW,
                                16, 16)
# libtcod.console_set_custom_font('dat/fonts/roger.png', libtcod.FONT_TYPE_GREYSCALE | libtcod.FONT_LAYOUT_TCOD, 32, 70)

libtcod.console_init_root(SCREEN_WIDTH, SCREEN_HEIGHT, 'Star Wars: A Rogue\'s Story', False, libtcod.RENDERER_SDL)
